...
// Only parsed bodies are retained. Headers are consumed for HTTP framing and
// never included in the request record or diagnostics.
...
TEST_CASE(interactive_executable_keeps_two_utf8_turns_and_linked_logs) {
    // Preserve case-level evidence even if an outer process deadline expires.
    ...
    // The server stays available for the bounded process lifetime, including a
    // cold first launch of a freshly copied executable.
...
TEST_CASE(interactive_executable_wires_compaction_memory_prompt_and_exact_credential_protection) {
    for (const bool enabled : {true, false}) {
        ...
        // Explicit synthetic configuration avoids discovery of any developer file.
...
TEST_CASE(interactive_executable_remembers_across_sessions_restarts_compacts_and_forgets) {
    // Missing Session-to-Runtime prompts, in-memory-only storage, dropped exact
    // tails, ignored toggles, or skipped checkpoints must each break this proof.
    ...
    {
        // A completed Session is intentionally left unconsolidated by turning
        // automatic memory off before exit; the next process must catch it up.
...
TEST_CASE(interactive_executable_discovers_colocated_env_from_another_working_directory) {
    // A cwd-only config search or a package with missing DLLs must fail here.
    ...
    if (std::getenv("AGENT_PROCESS_TEST_EXE") != nullptr) {
        // Attribute checks only: never open a staged credential file.
