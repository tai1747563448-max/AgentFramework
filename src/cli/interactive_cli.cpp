...
Result<std::vector<MemoryEntry>> InteractiveCli::eligible_memories(const SessionState& session) {
    ...
    try {
        ...
        // No adapter or provider diagnostics are terminal-safe.
    }
...
void InteractiveCli::consolidate(const SessionState& session) {
    ...
    try {
        ...
        // A boundary must remain usable when consolidation fails or throws.
    }
...
int InteractiveCli::run() {
    ...
    // MemoryEngine checks its durable checkpoint before any model work. Scan
    // every session, including interrupted ones, before choosing the latest.
