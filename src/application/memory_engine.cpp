...
Result<MemoryState> MemoryEngine::append_event(MemoryEventPayload payload) {
    // Read immediately before every append; do not reuse an earlier snapshot's sequence.
    ...
    if (auto* upsert = std::get_if<MemoryUpsertedPayload>(&payload)) {
        // These operations only create entries. A colliding seed cannot update or
        // resurrect an existing identity, even if the reducer supports updates.
...
Result<MemoryEntry> MemoryEngine::remember(
    ...
    try {
        ...
        // The domain requires positive provenance even for an empty Session.
...
Result<MemoryConsolidationResult> MemoryEngine::consolidate(const std::string& session_id) {
    try {
        ...
        // The active projection may have dropped old turns. Select only exact
        // committed payloads from the original, fully validated event stream.
        ...
        while (cursor < through) {
            ...
            if (first_bytes > kMaxConsolidationTranscriptBytes - 2) {
                // A single oversized turn cannot be made safe by retrying the
                // same request forever. Preserve it in Session JSONL, skip
                // extraction, and advance the append-only checkpoint.
