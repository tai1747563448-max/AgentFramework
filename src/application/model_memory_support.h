...
// Private helpers shared by the two tool-free model services.
...
inline bool valid_turns(const std::vector<CommittedSessionTurn>& turns) {
    ...
    for (const auto& turn : turns) {
        // Failed Session turns have no committed transcript, so indexes may
        // have gaps. Only monotonic order and uniqueness are required here.
