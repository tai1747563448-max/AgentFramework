...
#if defined(_WIN32)
        // Windows path identity is case-insensitive. ASCII folding gives drive
        // letters and ordinary path aliases one stable memory-scope spelling.
...
std::size_t value_bytes(const Value& value) {
    ...
    // An upper estimate for JSON scalars, including the longest finite double.
    return 32;
}
    ...
    if (context_.enabled &&
        ...
        try {
            ...
            if (!summary.has_value() || summary.value().size() > context_.max_summary_bytes) {
                ...
                // append_event validates the result through the reducer before I/O.
...
SessionTurnResult SessionEngine::recover_pending_turn(
    ...
    // Runtime recovery reuses a persisted model prompt if there is one. Only
    // reconstruct a fallback when the task stopped before its first model call.
